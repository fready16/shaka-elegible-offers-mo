...
public class ResponseFiller {
	...
	*/
	private void categoryLevel(CategoryListType category) {
		...
		offerings = category.getOfferings().stream().map(this::offeringsLevel).collect(Collectors.toList());
	...
	* @return OfferingType
	*/
	private OfferingType offeringsLevel(OfferingTypeOfferType offerType) {
		...
		offering.setProductOfferingPrice(this.getProductOfferingPriceList(offerType.getPlanBoList()));
	...
	*/
	private List<ComponentProdOfferPriceType> getProductOfferingPriceList(List<PlanBODetailsType> planBoList){
		return planBoList.stream().map(this::getProductOfferingPrice).collect(Collectors.toList());
	}
	...
	*/
	private List<KeyValueType> getPricedComponents(String poPriceId){
		...
		kvtPenalty.setValue(verifyPenalty.validate(meor, billingMethod, devicePenaltyInfo, businessRef, poPriceId));//LOGICA DE EXONERAR PENALIDADES
		return Arrays.asList(kvtPPF, kvtPenalty);
	}

	/**
	* Metodo que retorna la respuesta del servicio de AMDOCS si cumple con los parametros.
	* 
	* @param 
	...
	*/
	private DevicePenaltyInfo ifCallPenalty() {
		...
		try {
		    return penalty.consult(meor.getProductObj().getPublicId()).getRetrieveCommitmentPenaltyResponseData().getDevicePenaltyInfo();
